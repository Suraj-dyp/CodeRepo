...
public class ReportByStorage extends ReportUtility{
	...
	String query = "SELECT E.date AS COB_DATE, E.storage AS STORAGE, sum(E.quantity) AS QUANTITY, E.uom AS UNIT_OF_MEASUREMENT, sum(E.pnl_amount) AS PROFIT_LOSS, E.pnl_currency AS PROFIT_LOSS_CURRENCY " + 
			" FROM end_of_day AS E where E.storage IS NOT NULL AND E.date = ? GROUP BY E.storage";
